...
var deTokenizeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		endpoint := GetEndPoint("", "1.0", "detoken")
...
func init() {
	...
	deTokenizeCmd.Flags().StringP("policyName", "n", "",
		"Name of the policy to be used to tokenization")
	deTokenizeCmd.Flags().StringP("tokenData", "d", "",
		"Data to be detokenized")
	deTokenizeCmd.Flags().StringP("keyGuid", "k", "",
		"If you want to de-tokenize data using specific version of the key. If not provided, latest key version will be used")

	deTokenizeCmd.MarkFlagRequired("policyName")
	deTokenizeCmd.MarkFlagRequired("tokenData")
}
